...
export default function KhaltiCallbackPage() {
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      if (!pidx) {
        setState({ status: 'error', message: 'Missing Khalti payment reference.' })
        return
      }
      ...
      try {
        ...
        setState({
          status: 'success',
          message: 'Your Khalti payment was verified and your Pro plan is now active.',
        })
      } catch (error: any) {
        setState({
          status: 'error',
          message: error?.error || error?.message || 'Unable to verify Khalti payment.',
        })
      }
    }
    ...
    <main className="min-h-screen bg-gray-50 px-6 py-16">
      <div className="mx-auto max-w-xl rounded-3xl border border-gray-200 bg-white p-8 text-center shadow-sm">
        ...
        <h1 className="mt-4 text-2xl font-bold text-gray-900">
          {state.status === 'loading' ? 'Verifying Khalti payment' : state.status === 'success' ? 'Payment successful' : 'Payment verification failed'}
        </h1>
        <p className="mt-3 text-sm text-gray-600">
          {state.status === 'loading'
            ? 'Please wait while we confirm your payment with Khalti and activate your plan.'
            : state.message}
        </p>
