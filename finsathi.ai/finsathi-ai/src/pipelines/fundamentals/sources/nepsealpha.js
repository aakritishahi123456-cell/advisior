...
function buildCompanyUrl(symbol) {
  const base = process.env.NEPSEALPHA_BASE_URL || 'https://nepsealpha.com';
  const path = process.env.NEPSEALPHA_COMPANY_PATH || '/trading/company';
  return `${base.replace(/\/$/, '')}${path}/${encodeURIComponent(symbol)}`;
}
...
function parseNepsealphaFundamentals(html, symbol) {
  const $ = loadHtml(html);
